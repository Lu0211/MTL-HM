...
# -------------------------- 1. Config class (aligned with domain-adversarial + no-IDM training) --------------------------
class Config:
    # Core input/output configuration (variable-length compatible)
    ...
    # Domain-adversarial training domain configuration (fully aligned with training code)
    ...
    # Dataset acceleration limits (training + test + default fallback)
    dataset_acc_limits = {
    ...
    # No-IDM configuration (fully aligned with domain-adversarial training code)
    idm_params = {}
    idm_weight = 0.0  # set IDM weight to 0 to fully disable IDM fusion

    # Model parameters (fully aligned with domain-adversarial training code)
    hidden_size = 64
    enc_layers = 2
    dec_layers = 2
    dropout = 0.1
    meta_alpha = 0.01
    freeze_layers = 0  # layer freezing disabled during domain-adversarial training

    # Test configuration
    batch_size = 1  # must be 1 for variable-length samples
    collision_threshold = 0.0

    # Path configuration for domain-adversarial + no-IDM model
    save_dir = "output/our_method_ablation/only_adv"
    result_dir = os.path.join(save_dir, "test_result_adv_highd")
    data_paths = {
...
# -------------------------- 2. Utility functions (aligned with domain-adversarial training physics) --------------------------
def integrate_speed(acc_seq, speed_init, dt, pred_len):
    """Integrate acceleration to obtain speed sequence (variable-length, aligned with domain-adversarial code)."""
...
def calculate_ttc(spacing_seq, fv_speed_seq, lv_speed_seq, pred_len):
    """Compute Time-To-Collision (TTC) for variable-length sequences."""
...
# -------------------------- 3. No-IDM spacing integration (fully aligned with domain-adversarial training) --------------------------
def calculate_spacing_from_acc(pred_acc, ego_speed_init, front_speed_seq, init_spacing, dt, domain, pred_len):
    """Pure kinematic spacing integration (no IDM safety constraints, fully aligned with domain-adversarial code)."""
...
# -------------------------- 4. Dataset class (compatible with domain-adversarial model input) --------------------------
class MultiDomainTestDataset(Dataset):
    def __init__(self, domain, scaler):
        self.domain = domain
        self.scaler = scaler
        self.data_path = config.data_paths[domain]["test"]
        assert os.path.exists(self.data_path), f"[{domain}] Test dataset not found: {self.data_path}"
    ...
    def _process_data(self):
        """Fix core logic: align feature processing with domain-adversarial training code."""
        features = []
        labels = []
        meta = []
        raw_spacing = []
...
# -------------------------- 5. Model definition (fully aligned with domain-adversarial + no-IDM training) --------------------------
...
# -------------------------- 6. Evaluation function (domain-adversarial + no-IDM) --------------------------
def evaluate_model(domain, model, dataloader, device):
    ...
    with torch.no_grad():
        pbar = tqdm(dataloader, desc=f"Evaluating {domain.upper()} (Domain Adv + No IDM)")
        for batch in pbar:
    ...
    if metrics["pred_len_stats"]:
        print(f"\n[{domain}] Prediction length statistics (Domain Adv + No IDM):")
        print(
            f"  Min: {min(metrics['pred_len_stats'])} | Max: {max(metrics['pred_len_stats'])} | "
...
# -------------------------- 7. Main testing pipeline (domain-adversarial + no-IDM) --------------------------
def main():
    print("=" * 80)
    print("Testing Domain-Adversarial + No-IDM Transfer Learning Model - HighD Dataset")
    print(f"Training domains: {config.source_domains} | Test domains: {config.test_domains}")
    print("Domain-Adversarial Training: Enabled | IDM Fusion: Disabled | Layer Freezing: Disabled")
    print("=" * 80)

    model_path = os.path.join(config.save_dir, "pretrain_best.pth")
    assert os.path.exists(model_path), f"Domain-adversarial model not found: {model_path}"

    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    print(f"\n1. Loading domain-adversarial model on device: {device}")
    ...
    print(f"Successfully loaded domain-adversarial model weights (best loss: {checkpoint.get('best_loss', 'N/A')})")

    all_results = {}
    result_df = pd.DataFrame(columns=[
        "Dataset", "Valid Samples", "Total Prediction Steps", "Avg Prediction Length",
        "Acceleration MSE", "Acceleration MAE", "Speed MAE (m/s)", "Abs Jerk MAE (m/s³)",
    ...
    for domain in config.test_domains:
        print(f"\n{'=' * 60}")
        print(f"Evaluating {domain.upper()} Dataset (Domain Adv + No IDM)")
        print(f"{'=' * 60}")

        try:
            test_dataset = MultiDomainTestDataset(domain, scaler)
            if len(test_dataset) == 0:
                print(f"[WARNING] No valid samples for {domain.upper()}, skipping.")
                continue
            ...
            print(f"\nResults for {domain.upper()} (Domain Adv + No IDM):")
            print(
            ...
            print(f"[ERROR] Failed to evaluate {domain.upper()}: {str(e)}")
            import traceback
            traceback.print_exc()
            continue

    if not result_df.empty:
        result_excel_path = os.path.join(config.result_dir, "test_results_highd_adv_no_idm.xlsx")
        result_df.to_excel(result_excel_path, index=False, engine="openpyxl")
        print(f"\nTest results saved to: {result_excel_path}")

        print(f"\n{'=' * 80}")
        print("Domain-Adversarial + No-IDM Model Test Summary (HighD Dataset)")
        print(f"{'=' * 80}")
        print(result_df.to_string(index=False))

        fig, axes = plt.subplots(2, 3, figsize=(18, 10))
        fig.suptitle("Domain-Adversarial + No-IDM Transfer Learning Performance - HighD Dataset",
        ...
        plt.tight_layout()
        fig_path = os.path.join(config.result_dir, "test_performance_highd_adv_no_idm.png")
        plt.savefig(fig_path, dpi=300, bbox_inches="tight")
        plt.close()
        print(f"Performance visualization saved to: {fig_path}")

    print(f"\n{'=' * 80}")
    print("HighD Dataset Domain-Adversarial + No-IDM Variable-Length Testing Completed!")
