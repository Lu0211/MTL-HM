...
# -------------------------- 1. Hyperparameter configuration (domain adversarial only) --------------------------
class Config:
    ...
    step_pred = 10  # number of steps per recurrent prediction
    ...
    # Training configuration (domain adversarial only)
    pretrain_epochs = 80
    finetune_epochs = 0
    batch_size = 256
    pretrain_lr = 0.001
...
def calculate_spacing_from_acc(pred_acc, ego_speed_init, front_speed_seq, init_spacing, dt, domain):
    ...
    s0 = torch.tensor(0.1, dtype=pred_acc.dtype, device=device)
...
# -------------------------- 5. Model definition (domain adversarial only, no layer freezing) --------------------------
...
class DomainDiscriminator(nn.Module):
    """Keep domain discriminator (core component)."""
...
class Seq2SeqFollowingModel(nn.Module):
    def __init__(self, config):
        ...
        # Disable layer freezing (all layers trainable)
    ...
    def _build_new_input(self, current_x, step_pred, current_domain, front_speed_seq=None):
        """History window update without IDM."""
...
# -------------------------- 6. Training and validation functions (domain adversarial only) --------------------------
def train_one_epoch(model, loader, optimizer, epoch, stage="pretrain"):
    ...
    for batch in pbar:
        ...
        loss_gen = 0.2 * loss_acc + 0.8 * loss_spacing + 0.1 * loss_adv
    ...
    print(f"  {stage} | Time: {epoch_duration:.2f} min | Acc MSE: {avg_acc_loss:.6f} | Spacing MSE: {avg_spacing_loss:.6f}")
...
# -------------------------- 7. Main training pipeline (domain adversarial pretraining only) --------------------------
if __name__ == "__main__":
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    model = Seq2SeqFollowingModel(config).to(device)
    print(f"Model loaded to device: {device}")
    print("Ablation study: domain adversarial only (no IDM + no layer freezing + no finetuning)")
    ...
    if not pretrain_done:
        print("\n" + "=" * 60)
        print("Stage 1: Source domain pretraining (domain adversarial only)")
        print("=" * 60)
        ...
        for epoch in range(config.pretrain_epochs):
            ...
            if target_avg_loss < best_pretrain_loss:
                ...
                print(f"  Best pretrain model saved! Loss: {best_pretrain_loss:.6f}")
            else:
                pretrain_early_stop += 1
                if pretrain_early_stop >= config.patience:
                    print("  Pretraining early stopping triggered!")
                    break
    else:
        ...
        pretrain_log = []
        total_start_time = time.time()
        print("Pretrained model already exists, loading best weights directly!")
    ...
    plt.xlabel("Epoch")
    plt.ylabel("Target domain avg loss")
    plt.title("Domain Adversarial Only - Pretrain Loss Curve (No IDM)")
    plt.legend()
    plt.grid(alpha=0.3)
    ...
    plt.xlabel("Dataset")
    plt.ylabel("Spacing MSE (m^2)")
    plt.title("Domain Adversarial Only - SPMD Spacing Loss (No IDM)")
    ...
    plt.xlabel("Epoch")
    plt.ylabel("Time (min)")
    plt.title("Domain Adversarial Only - Training Time Curve (No IDM)")
    plt.legend()
    plt.grid(alpha=0.3)
    ...
    # Final statistics
    print("\n" + "=" * 60)
    print("Training completed! Final results (domain adversarial only)")
    print("=" * 60)
    print(f"Best pretrain loss: {best_pretrain_loss:.6f}")
    print(f"Total training time: {total_training_time:.2f} min")
    print("Pretrain model: pretrain_best.pth")
    print("Training log: training_time_log.csv")
    print("Results visualization: training_results.png")
