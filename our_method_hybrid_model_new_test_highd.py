...
# -------------------------- 1. Configuration Class (Retain training domains, add new test domains) --------------------------
class Config:
    ...
    # seq_len = 250  # Fixed historical length (first 100 timesteps)
    dt = 0.04  # Fixed 10Hz time interval (0.1s per step)
    # Ablation experiments for different history & prediction lengths
    seq_len = 250  # 25, 75, 125, 200, 250(1,3,5,8,10)
    # 5s:125,4s:100,3s:75,2s:50,1s:25，6s:150,7s:175
    step_pred = 3  #10，15，20，25, 30,45，75,100,200),0.4,0.6,0.8,1,1.2,1.8,3,4,8

    # Variable-length configuration for HighD
    min_total_len = 375  # Minimum total sample length (100 history steps + min 50 prediction steps + 1)
    min_pred_len = 15  # Minimum number of acceleration prediction timesteps
    max_pred_len = 125  # Maximum number of acceleration prediction timesteps
    # 30highd
    # min_total_len = 750  # Minimum total sample length (100 history steps + min 50 prediction steps + 1)
    # min_pred_len = 200  # Minimum number of acceleration prediction timesteps
    # max_pred_len = 200  # Maximum number of acceleration prediction timesteps
    # step_pred = 25  # Predict 1 second per sliding window iteration

    # Retain original training domain list to match discriminator dimension
    domains = ["ngsim", "lyft", "waymo", "spmd1", "spmd2"]
    source_domains = ["ngsim", "spmd2"]
    target_domains = ["spmd1", "lyft", "waymo"]

    # New dedicated test domains
    # test_domains = ["highd", "30highd"]
    test_domains = ["highd"]
    # test_domains = ["30highd"]

    # Acceleration constraints for each dataset with default fallback
    dataset_acc_limits = {
    ...
    # IDM parameters with default fallback
    idm_params = {
        "highd": {"v0": 31.63, "T": 0.85, "a": 0.36, "b": 0.39, "s0": 3.50, "delta": 2.89},
        # "30highd": {"v0": 16.47, "T": 1.62, "a": 0.85, "b": 0.29, "s0": 1.35, "delta": 2.84},
        "default": {"v0": 38.11, "T": 1.39, "a": 1.66, "b": 1.18, "s0": 6.63, "delta": 1.57}
    }
    # idm_weight = 0.3  # Fusion weight for IDM acceleration
    idm_weight = 0.1

    # Model hyperparameters fully consistent with training script
    hidden_size = 64
    enc_layers = 2
    dec_layers = 2
    dropout = 0.1
    ...
    batch_size = 1  # Must set to 1 to avoid dimension mismatch for variable-length samples
    collision_threshold = 0.0  # Threshold to detect collision

    save_dir = "output/our_method_with_IDM_final_new"
    # result_dir = os.path.join(save_dir, "test_results_highd")
    # result_dir = os.path.join(save_dir, "ablation_test_result_highd") # Ablation for different λ values
    # result_dir = os.path.join(save_dir, "different_history_step_test_result_highd")  # Different history length experiments
    result_dir = os.path.join(save_dir, "different_slipe_step_test_result_highd")  # Different sliding window length ablation
    data_paths = {
...
# -------------------------- 2. Utility Functions (Variable-length compatible) --------------------------
...
# -------------------------- 2. Core IDM Calculation Functions (Variable-length compatible) --------------------------
def idm_desired_spacing(ego_speed, front_speed, params):
    """Calculate desired safe gap of IDM model"""
...
def idm_acceleration(ego_speed, front_speed, current_spacing, params):
    """Compute IDM predicted acceleration with vectorized operation"""
...
def calculate_spacing_from_acc(pred_acc, ego_speed_init, front_speed_seq, init_spacing, dt, domain, pred_len):
    """Integrate gap distance with IDM physical constraints for variable-length sequences"""
    ...
    idm_params = config.idm_params.get(domain, config.idm_params["default"])
    ...
    for t in range(1, speed_len):
        ...
        # Fetch leading vehicle speed, avoid out-of-bounds for variable-length sequences
    ...
    # Return gap sequence aligned with acceleration length (take last pred_len steps)
    return spacing_pred[:, 1:1 + pred_len]


# -------------------------- 3. Dataset Class (Core fix for length alignment) --------------------------
class MultiDomainTestDataset(Dataset):
    ...
    def _process_data(self):
        """Core logic: strictly align acceleration label length with prediction length"""
        features = []  # Fixed dimension [4, 250]
        labels = []  # Variable-length acceleration sequence [pred_len]
        meta = []  # Fixed 5-dimensional scene statistical features
        raw_spacing = []  # Variable-length ground truth gap sequence
        raw_fv_speed = []  # Variable-length following vehicle speed
        raw_lv_speed = []  # Variable-length leading vehicle speed
        ...
        for sample in self.raw_data:
            try:
                ...
                # Filter samples shorter than minimum total length
                ...
                # Extract ground truth window matched with prediction range
        ...
        # Convert fixed-length lists to numpy array
    ...
    def __getitem__(self, idx):
        ...
        return {
            "x": torch.FloatTensor(self.features[idx]),  # Fixed [4,250] historical input
...
# -------------------------- 4. Model Definition (Fully consistent with training script) --------------------------
...
class LearnableWeights(nn.Module):
    """Adaptive multi-task loss weight learning module"""
    def __init__(self, num_tasks=3):
        super().__init__()
        # Learnable log variance to ensure positive weight values
        self.log_vars = nn.Parameter(torch.zeros(num_tasks))

    def forward(self, losses):
        """
        losses: [loss_acc, loss_spacing, loss_adv]
        Return weighted total loss plus regularization term
        """
        total_loss = 0
        regularization = 0
        ...
        # Total loss = weighted loss sum + regularization penalty
        return total_loss
class Seq2SeqFollowingModel(nn.Module):
    def __init__(self, config):
        ...
        # Domain discriminator dimension remains consistent with training domain count
        self.domain_discriminator = DomainDiscriminator(config.hidden_size, len(config.domains))
        # Initialize dataset acceleration limits with default fallback
    ...
    def step_predict(self, enc_hidden, enc_cell, meta_feat, current_domain, step_num,
        ...
        """Single sliding window iterative prediction for variable-length output"""
        ...
        idm_params = self.config.idm_params.get(current_domain, self.config.idm_params["default"])
        ...
        for t in range(step_num):
            ...
            # Fuse IDM acceleration if trajectory data is available
    ...
    def forward(self, x, meta, current_domain, pred_len,
        ...
        # Concatenate and trim prediction sequence to target length
    ...
    def _build_new_input(self, current_x, step_pred, current_domain, front_speed_seq=None, step_num=None):
        """Reconstruct sliding historical window after each prediction iteration"""
...
def evaluate_model(domain, model, device):
    model.eval()
    metrics = {
    ...
    with torch.no_grad():
        ...
        for batch in pbar:
            try:
                ...
                assert y_true.shape[1] == pred_len, f"Label length mismatch with prediction length"
...
# -------------------------- 6. Main Test Pipeline (Iterate over dedicated test domains only) --------------------------
def main():
    # Configuration header print
    print("=" * 80)
    print("Start testing IDM-fused Seq2Seq model on new test domains (HighD/30HighD) with variable-length prediction")
    print(f"Model training domains: {config.domains} | Test domains: {config.test_domains}")
    print("=" * 80)

    model_path = os.path.join(config.save_dir, "finetune_best.pth")
    if not os.path.exists(model_path):
        raise FileNotFoundError(f"Model checkpoint file not found at path: {model_path}")

    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    print(f"\n1. Loading trained model to computing device: {device}")
    ...
    print(f"Layer freeze count: {config.freeze_layers} | IDM fusion weight: {config.idm_weight}")
    ...
    result_df = pd.DataFrame(columns=[
        ...
        "Avg Pred Absolute Jerk", "Max Pred Absolute Jerk", "Std Pred Absolute Jerk",
        "Gap MSE (m²)", "Gap MAE (m)", "Avg Minimum TTC (s)",
        "Collision Count", "Collision Rate (%)"
    ])

    # Only loop dedicated test domains instead of all training domains
    for domain in config.test_domains:
        print(f"\n{'=' * 60}")
        print(f"Evaluating {domain.upper()} test dataset")
        print(f"{'=' * 60}")
        try:
            test_dataset = MultiDomainTestDataset(domain, scaler)
            if len(test_dataset) == 0:
                print(f"⚠️ No valid samples found for {domain.upper()}, skip evaluation")
                continue
            ...
            print(f"Average predicted absolute jerk: {metrics['avg_pred_abs_jerk']:.6f} m/s³")
            print(f"Maximum predicted absolute jerk: {metrics['max_pred_abs_jerk']:.6f} m/s³")
    ...
    if not result_df.empty:
        excel_save_path = os.path.join(config.result_dir, "test_results_highd_30highd.xlsx")
        result_df.to_excel(excel_path, index=False, engine="openpyxl")
        print(f"\n✅ Test result table saved to {excel_path}")

        print(f"\n{'=' * 80}")
        print("Overall performance summary for new test domains")
        print(f"{'=' * 80}")
        print(result_df.to_string(index=False))

        fig, axes = plt.subplots(2, 3, figsize=(18, 10))
        fig.suptitle("Performance of IDM-fused Seq2Seq Model on New Test Domains (HighD/30HighD)", fontsize=16, fontweight="bold")
        ...
        plt.tight_layout()
        fig_save_path = os.path.join(config.result_dir, "test_performance_highd_30highd.png")
        plt.savefig(fig_save_path, dpi=300, bbox_inches="tight")
        plt.close()
        print(f"✅ Performance visualization figure saved to {fig_save_path}")

    print(f"\n{'=' * 80}")
    print("Variable-length prediction test pipeline completed for new domains (HighD/30HighD)")
